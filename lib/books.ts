...
const titleNotes: Record<string, Partial<Pick<Book, "summary" | "detail" | "relevance">>> = {
...
export function commerceLinks(book: Pick<Book, "title">) {
  const query = encodeURIComponent(`${book.title} book`);
